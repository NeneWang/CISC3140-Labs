# Gets the value inside the cell, ""
func getValue(stringIn){
  return substr(stringIn,2, length(stringIn)-2)
}

#declaration of initial variables and csv parsing method.
BEGIN {FS = ",";
...
#Runs for every row
{
# Ignores the first row (of titles)
if(NR>1) boro = getValue($2);


#If the row indicates is from Brooklyn, Run the algorithm to compute Brooklyn Statistics by the end
if(boro == boro_brooklyn)
{
  summation_fullval_brooklyn += getValue($13); count_target_brooklyn++ ;
...
#If the row indicates is from Manhattan, Run the algorithm to compute Manhattan Statistics by the end
if(boro == boro_manhattan) {
  summation_fullval_manhattan += getValue($13); count_target_manhattan++ ;
...
#If the row indicates is from Bronx, Run the algorithm to compute Bronx Statistics by the end
 if(boro == boro_bronx) {

 summation_fullval_bronx += getValue($13); count_target_bronx++;
